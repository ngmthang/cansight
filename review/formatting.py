...
def format_wall(wall: Wall, unit_system: str = "us") -> str:
    ...
    return (
        f"Wall {wall.id}: "
        f"{units.format_length(length, unit_system)} long, "
...
def format_door(door: Door, unit_system: str = "us") -> str:
    return (
        f"Door {door.id}: "
        f"{units.format_length(door.width, unit_system)} wide x "
...
def format_window(window: Window, unit_system: str = "us") -> str:
    return (
        f"Window {window.id}: "
        f"{units.format_length(window.width, unit_system)} wide x "
...
def format_room(room: Room, unit_system: str = "us") -> str:
    return (
        ...
        f"{units.format_length(room.height(), unit_system)} ceiling "
        f"(confidence: {_pct(room.confidence)})"
    )


def format_floor(floor: Floor, unit_system: str = "us") -> str:
    return (
        f"Floor {floor.id}: "
        f"{units.format_area(floor.area(), unit_system)} "
        f"(confidence: {_pct(floor.confidence)})"
    )


def format_ceiling(ceiling: Ceiling, unit_system: str = "us") -> str:
    return (
        f"Ceiling {ceiling.id}: "
        f"{units.format_area(ceiling.area(), unit_system)} "
        f"(confidence: {_pct(ceiling.confidence)})"
    )
...
def format_object(obj: BuildingObject, unit_system: str = "us") -> str:
    ...
    return (
        f"{obj.type.value} {obj.id} "
        f"(confidence: {_pct(obj.confidence)})"
    )