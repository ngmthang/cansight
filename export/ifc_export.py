"""
    ...
    Mapping used:
      ...
      Room   -> IfcSpace
      Floor/Ceiling -> IfcSlab
      Level  -> IfcBuildingStorey

    This is intentionally geometry-simple (rectangular extrusions) for V1 —
...
def export_to_ifc(model: BuildingModel, path: str) -> list[str]:
    """Exports the model, returns a list of validation warnings
    (empty = clean)."""
    warnings: list[str] = []
    errors = model.validate()
    if errors:
        # Do not silently export an inconsistent model -- per spec
        # Section 13,
        # export goes through schema validation, not just IFC schema
        # validation.
        raise ValueError(
            f"Building Model failed validation, refusing export: {errors}"
        )
    ...
    for obj_id, obj in model.objects.items():
        if isinstance(obj, (Door, Window)):
            storey = storeys[obj.level]
            ifc_class = (
                "IfcDoor" if isinstance(obj, Door) else "IfcWindow"
            )
            ...
            ifcopenshell.api.run(
                ...
                ifc,
                relating_structure=storey,
                products=[opening_elem],
            )
            host_wall = wall_entities.get(obj.host_wall_id)
            if host_wall is None:
                warnings.append(
                    f"{obj_id}: host wall not exported, "
                    f"skipping fill relationship"
                )
                continue
            ifcopenshell.api.run(
                "feature.add_filling",
                ifc,
                opening=_ensure_opening(ifc, host_wall),
                element=opening_elem,
            )

    for obj_id, obj in model.objects.items():
        if isinstance(obj, (Floor, Ceiling)):
            storey = storeys[obj.level]
            ...
            # IFC has no dedicated "CEILING" predefined type;
            # ROOF is the closest horizontal-slab semantic for
            # the underside-of-structure-above case
            slab.PredefinedType = (
                "FLOOR" if isinstance(obj, Floor) else "ROOF"
            )
            ifcopenshell.api.run(
                "spatial.assign_container",
                ifc,
    ...
    for obj_id, obj in model.objects.items():
        if isinstance(obj, Room):
            ...
            # IfcSpace is itself a spatial structure element, so it
            # nests
...
def _ensure_opening(ifc, wall):
    """Create (or reuse) an IfcOpeningElement in the wall for a
    fill relationship."""
    key = wall.id()
    if key in _opening_cache:
        return _opening_cache[key]
    opening = ifcopenshell.api.run(
        "root.create_entity", ifc, ifc_class="IfcOpeningElement"
    )
    ifcopenshell.api.run(
        "feature.add_feature", ifc, feature=opening, element=wall
    )
