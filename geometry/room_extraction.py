"""
    Problem: given a set of wall centerlines (a planar graph - walls are edges,
    wall endpoints are vertices), find the enclosed ROOM POLYGONS.

    This is the classic "find faces of a planar straight-line graph" problem,
    ...
    junction (T-junctions, 4-wall intersections) - ordinary cycle-finding
...
def _round_pt(p: Point, precision: int = 4) -> Point:
    """Snap endpoints so shared corners compare equal despite float noise."""
...
def _split_at_t_junctions(walls: list[Wall], tol: float = 0.02) -> list[Wall]:
    """
    ...
    def project_param(p: Point, a: Point, b: Point) -> float | None:
        ...
        if t <= 1e-6 or t >= 1 - 1e-6:
            return None # at/beyond the endpoints -> not a T-junction, handled by normal adjacency
    ...
    new_walls: list[Wall] = []
    for (a, b) in walls:
        # map param t -> the ACTUAL neighboring endpoint (not its projection),
        # so the new vertex exactly coincides with that wall's own endpoint
        # coordinate -- otherwise the two "shared" points differ by the
        # perpendicular noise offset and never match after rouding.
...
def extract_rooms(walls: list[Wall]) -> list[list[Point]]:
    ...
    # 2. at each vertex, order neighbor by angle (needed for "next half-edge" step)
    ...
    for v, nbrs in adjacency.items():
        sorted_neighbors[v] = sorted(set(nbrs), key=lambda n: angle_from(v, n))
    ...
    def next_half_edge(u: Point, v: Point) -> Point:
        """Given we arrived at v via u->v, pick next vertex w such that
        v->w is the next edge clockwise from v->u (standard face-tracing rule)."""
        nbrs = sorted_neighbors[v]
        incoming_angle = angle_from(v, u)
        # find nbrs sorted position of u, then step to the next one (wrap around)
        # this yields the tightest clockwise turn, which traces faces correctly
        angles = [(angle_from(v, n), n) for n in nbrs]
        angles.sort()
        idx = next(i for i, (ang, n) in enumerate(angles) if n == u)
        return angles[(idx - 1) % len(angles)][1] # step counter-clockwise in angle list = clockwise turn
        ...
        for start_u, start_v in [(a, b), (b, a)]:
            ...
            while True:
                ...
                if len(face) > 4 * len(edges) + 10:
                    break # safety valve against malformed input (dangling walls etc.)
